# 多UAV追逃项目技术文档（light_mappo）

本文档面向当前仓库的真实实现，重点对应：
- 环境：`envs/env_uav_pursuit.py`、`envs/env_continuous.py`
- 训练与评估入口：`train/train.py`、`train/eval.py`
- 任务执行与日志：`runner/uav/role_runner.py`
...
其核心研究问题是：
- 在连续动作空间中，如何让多智能体形成稳定协同而非互相干扰；
- 在局部观测和时变可见性下，如何提升追捕效率与鲁棒性；
- 如何通过角色化训练框架同时支持 Hunter/Target 的策略学习与对抗评估。
...
- **近实战初始化**：支持 Hunter 区域编队起飞（zone 随机分布），并可约束 Target 避开该区域。
